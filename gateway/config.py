    ...
    def get_connected_platforms(self) -> List[Platform]:
        ...
        for platform, config in self.platforms.items():
            ...
            # WeCom uses extra dict for bot credentials
                ...
                connected.append(platform)
            # Viche uses enabled flag only (VICHE_ENABLED activates it)
            elif platform == Platform.VICHE:
                connected.append(platform)
        return connected
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    # Viche AI agent network
    viche_enabled = os.getenv("VICHE_ENABLED", "").lower() in ("true", "1", "yes")
    if viche_enabled:
        if Platform.VICHE not in config.platforms:
            config.platforms[Platform.VICHE] = PlatformConfig()
        config.platforms[Platform.VICHE].enabled = True
        registry_url = os.getenv("VICHE_REGISTRY_URL", "")
        if registry_url:
            config.platforms[Platform.VICHE].extra["registry_url"] = registry_url
        agent_name = os.getenv("VICHE_AGENT_NAME", "")
        if agent_name:
            config.platforms[Platform.VICHE].extra["agent_name"] = agent_name
        capabilities = os.getenv("VICHE_CAPABILITIES", "")
        if capabilities:
            config.platforms[Platform.VICHE].extra["capabilities"] = capabilities
    viche_home = os.getenv("VICHE_HOME_CHANNEL")
    if viche_home and Platform.VICHE in config.platforms:
        config.platforms[Platform.VICHE].home_channel = HomeChannel(
            platform=Platform.VICHE,
            chat_id=f"viche:{viche_home}",
            name=os.getenv("VICHE_HOME_CHANNEL_NAME", "Home"),
        )
